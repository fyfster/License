...
if( 0 == $theId){
  $smarty->setSessionNotification('Tipul de cerere cautata nu este disponibila', "error");
  header("location:index.php");
  return;
}
...
if('upload' == $theAction){
  ...
  $theNewName =  $_SESSION['user']['id']. "_C_" .rand(10,100). "." .$theExt; 
  $theTargetPath = $theUploadDemandPath . $theNewName;
  ...
  $theFormEntity->setPath('cereri/'. $theNewName);
  $theInsertedForm = $theFormModel->insert($theFormEntity);
  $smarty->setSessionNotification('Cererea a fost depusa', "success");
  header("location:index.php");
  return;
}
$smarty->assign("formTypeValue", "demand");
